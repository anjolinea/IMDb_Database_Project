...
JOIN MovieGenre ON Movie.movieID = MovieGenre.movieID
JOIN Genre ON MovieGenre.genreID = Genre.genreID
WHERE Actor.actorName LIKE '%%'
    AND Genre.genreName LIKE '%F%' -- Fantasy, Sci-Fi
    AND Movie.movieTitle LIKE '%%'
    AND Movie.movieRating >= 5;
...
JOIN MovieGenre ON Movie.movieID = MovieGenre.movieID
JOIN Genre ON MovieGenre.genreID = Genre.genreID
WHERE Actor.actorName LIKE '%%' -- all movies
    AND Genre.genreName LIKE '%%' 
    AND Movie.movieTitle LIKE '%%'
    AND Movie.movieRating >= 5;
...
JOIN MovieGenre ON Movie.movieID = MovieGenre.movieID
JOIN Genre ON MovieGenre.genreID = Genre.genreID
WHERE Actor.actorName LIKE '%Tom Holland%' -- no movies exist
    AND Genre.genreName LIKE '%%' 
    AND Movie.movieTitle LIKE '%Avatar%'
    AND Movie.movieRating >= 5;